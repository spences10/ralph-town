	...
	git: GitOperations = {
		clone: async (
			...
			if (token && url.includes('github.com')) {
				...
				const askpass_script = `${this.workspace}/.git-askpass.sh`;
				// Token is written to file, not interpolated into command
				await this.write_file(
					askpass_script,
					`#!/bin/sh\necho '${token.replace(/'/g, "'\\''")}'`,
				);
				await this.execute(`chmod +x ${quote([askpass_script])}`);
				...
				try {
					...
					await this.execute(`rm -f ${quote([askpass_script])}`);
				}
			} else {
				await this.execute(quote(args));
			}
		...
		push: async (branch: string, _token?: string) => {
			await this.execute(quote(['git', 'push', '-u', 'origin', branch]));
		},
		...
		create_worktree: async (branch: string): Promise<string> => {
			...
			await this.execute(
				quote(['git', 'worktree', 'add', worktree_path, '-b', branch]),
			);
